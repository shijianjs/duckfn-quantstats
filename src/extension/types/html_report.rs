...
// 结构体：`STRUCT(symbol, benchmark, strategy_title, html, file_path)[]`。
//
...
//
// 字段名就是 SQL 里的键名（duckfn 的 `DuckStruct` 派生不支持字段级改名）。`symbol` / `benchmark` /
// `strategy_title` / `html` 里只有 `benchmark` 可空 —— 没配基准时那份报告是单序列的，没有基准可写；
...
// **list** whose elements are this struct: `STRUCT(symbol, benchmark, strategy_title, html, file_path)[]`.
//
...
//
// The field names are the keys in SQL (duckfn's `DuckStruct` derive has no field-level renaming). Among
// `symbol` / `benchmark` / `strategy_title` / `html` only `benchmark` is nullable — with no benchmark
// configured that report is a single-series one and has no benchmark to name; `file_path` is nullable too —
// without `output_dir` (and without the browser) the report never leaves memory, so there is no path to
// report back.
//
...
/// 一份报告的结果行：`(标的, 基准, 显示名, 报告 HTML, 落盘路径)`。
///
/// One row of the report result: `(symbol, benchmark, display name, report HTML, written path)`.
